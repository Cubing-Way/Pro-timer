      ...
      html += `
                <div class="modal-solve-row">
                    <div class="modal-solve-main">
                        <span>#${wrapTimesValue$1(i + 1)}: ${formatStatValue(s)}</span>
...
function applyTimesColor(colorKey) {
  const colorValue = getThemeVar(`theme-text-${colorKey}`, "--text-main") || getComputedStyle(document.documentElement).getPropertyValue("--text-main");
...
function renderCurrentAverage(currentType, currentBlock) {
  ...
  if (currentType === "live") {
    ...
    averageObj.solvesArray.forEach((s, i) => {
      html += `
                <div class="history-solve">
                    ${wrapTimesValue(i + 1)} - ${formatStatValue(s)}
                </div>
            `;
    });
    html += `
    ...
    averageObj.solvesArray.slice().reverse().forEach((solve, index) => {
      html += `
                    <div class="history-block history-solve">
                        ${wrapTimesValue(averageObj.solvesArray.length - index)}
