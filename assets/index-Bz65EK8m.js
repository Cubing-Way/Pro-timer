...
function getStatisticsByDate(range) {
  const session2 = getCurrentSession();
  const allSolves = getAllSolvesFromSession(session2);
  const filteredSolves = filterSolvesByRange(allSolves);
  const stats = getStatisticsFromSolves(filteredSolves, session2);
  const filteredAvgs = filterAveragesByRange(session2);
...
function formatStatValue(value) {
  ...
  if (value.time && value.solvePhases) {
    ...
    const phases = value.solvePhases.length > 1 ? " | " + value.solvePhases.map((phase, index) => wrapTimesValue(index + 1) + ": " + phase).join(" | ") : "";
...
function renderHistoryList(averages, currentType) {
  ...
  let html = "";
  if (averageObj.mode === "classic") {
    averageObj.solvesArray.slice().reverse().forEach((solve, index) => {
      html += `
